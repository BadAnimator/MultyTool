...
class Logger:
    def __init__(self, logfile: str, _format="[%DATETIME%] %TEXT%", end_function=None):
        self.logfile = logfile
        self.formate = _format
        self.endfunc=end_function if callable(end_function) else None

    def log(self, *text):
        text = "".join(str(item) for item in text)
        text = self.formate.replace(
            "%DATETIME%",
            time.ctime(
                int(time.time())
            )
...
class Utils:
    def formatProxies(Proxies: list) -> dict:
            prefix = "https://t.me/proxy?"
            # Отбираем строки с правильным началом
            valid = [u for u in Proxies if u.startswith(prefix)]
            # Из каждой строки извлекаем параметры и формируем словарь
            return [
                {k: v for k, v in dict(p.split('=', 1) for p in url[len(prefix):].split('&')).items() if k in ('server', 'port', 'secret')}
                for url in valid
                if all(p in dict(p.split('=', 1) for p in url[len(prefix):].split('&')) for p in ('server', 'port', 'secret'))
            ]